    ...
    if path == "savetweet":
        if request.is_ajax():
            if request.method == 'POST':
                ...
                with open(os.path.join("app/twitterlike/twitterlikepages/static/", tempFileName), 'r') as file_json_data:
                ...
                with open(os.path.join("app/twitterlike/twitterlikepages/static/", tempFileName), 'w') as file_write_data:
                    file_write_data.write(json.dumps(file_data))
        return HttpResponse(status=200)
    else:
        jsonFileName = "data" + "/" + path + ".json"
        with open(os.path.join("app/twitterlike/twitterlikepages/static/", jsonFileName)) as json_data:
